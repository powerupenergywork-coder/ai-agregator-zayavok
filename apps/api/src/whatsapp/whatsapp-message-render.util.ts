...
export function renderReviewCard(order: OrderDto, lang: Language): OutgoingWhatsAppMessage {
  ...
  // Предупреждение ДО отправки, а не после первого звонка.
  //
  // Ожидание, выставленное заранее, работает несравнимо лучше объяснения
  // задним числом: заявка №137 — человек получил «отправили 5 исполнителям»,
  // через шестнадцать минут и тринадцать звонков написал «Всё ненада».
  const heads =
    lang === "kk"
      ? "Өтінімді бірнеше орындаушы алады — олар қоңырау шалып, өз бағасын айтады."
      : "Заявку получат несколько исполнителей — они позвонят и назовут свои цены.";
  const body =
    lang === "kk"
      ? `Өтінімді тексеріңіз:\n\n${lines.join("\n")}\n\n${heads}\n\nБәрі дұрыс па?`
      : `Проверьте заявку:\n\n${lines.join("\n")}\n\n${heads}\n\nВсё верно?`;
