...
Если срочно, звоните напрямую: ${env.supportPhone}`,
   ...
   *
   * Здесь отмена исполнителям — правильная и срочная: им назвали работу,
   * работы больше нет, и каждая минута молчания это звонок человеку, который
   * уже попрощался. Это единственный случай, когда такое письмо уместно —
   * в отличие от «нашёл исполнителя», где заявка выполнена, а не отменена.
   *
   * Клиенту говорим, что звонки прекратятся: он написал «не надо» именно
   * потому, что телефон не умолкает.
   */
  /**
  ...
  private async closeOrderAsNotNeeded(
    ...
    await this.whatsapp.sendText(
      phone,
      lang === "kk"
        ? `Түсіндім, №${orderNumber} өтінімді жаптым. Орындаушыларға хабарладым — енді қоңырау шалмайды.

Қажет болса — осында жазыңыз.`
        : `Понял, закрыл заявку №${orderNumber}. Сообщил исполнителям — больше звонить не будут.
