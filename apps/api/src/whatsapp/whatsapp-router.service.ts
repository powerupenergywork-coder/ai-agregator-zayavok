 ...
 */
// Порядок слов любой. Берик, 12 сентября: «Других заказов есть у вас»,
// «Заказ есть», «Еще заявка есть» — три вопроса о работе, и на каждый
// пришло «Записал по заявке №183»: прежний перечень ждал «есть» строго
// перед словом «заказ».
const MORE_ORDERS_RE =
  /есть\s*(ещ[её]|еще)?\s*(заказ|заявк|работ)|(заказ|заявк|работ)[а-яё]*\s+(есть|бар)|(други[хе]|ещ[её]|еще|новы[хе])\s+(заказ|заявк)|нужн[ыа]\s*заказ|дайте\s*(заказ|заявк)|тапсырыс бар ма|(если|когда|как)\s*(будет|будут|появ|поступ)|обращайтесь|обращайся|зовите|звоните|пишите|хабарласыңыз|болса айтыңыз/i;
/**
 ...
 */
/**
 * Глагол задачи: то, что человек хочет сделать с грузом, техникой или
 * помещением. Без «нужен»: клиент, описывающий работу подробно, слово
 * «нужен» часто пропускает — оно и так очевидно из описания.
 */
const TASK_RE =
  /вывоз|вывез|вывест|перевез|перевоз|переезд|поднять|поднят|погруз|разгруз|демонтаж|демонтир|доставить|доставк|увезти|забрать|убрать|уборк|подключ|установ|почин|отремонт|прочист|обрезать|спилить|эвакуац|эвакуир|шығару|тасымал|жеткізу|көшу/i;

/**
 * Признак места: город из справочника или адресная разметка. Вместе с
 * глаголом задачи это заявка даже без единого «нужен».
 */
// Границы слов явные: \b кириллицу не видит, и «ул. Абая» через \b не
// находился никогда.
const PLACE_RE =
  /(^|[\s,.;(])(ул|улиц[а-яё]*|пр|просп|проспект[а-яё]*|мкр|микрорайон[а-яё]*|жк|д|дом[а-яё]*|кв|этаж[а-яё]*|подъезд[а-яё]*|район[а-яё]*|пос|пос[её]л[оа]к|село|аул|көш[а-яё]*|ауд[а-яё]*)($|[\s.,;:)])|\d+\s*(этаж|подъезд)/i;

/**
 * Похоже ли на заявку. Три способа сказать «мне нужно»: словом «нужен»,
 * задачей с местом или задачей с категорией.
 *
 * Раньше хватало одного «нужен», а упоминание цены отменяло всё: +7 701
 * 840 65 70, 11 сентября — «Астана, ЖК ИНФИНИТИ 1. Нужно вывезти диван.
 * Просьба подсказать стоимость» — получил «вам нужна техника или вы сами
 * оказываете услуги?» и не ответил. +7 707 753 17 22 в тот же день описал
 * вывоз после ремонта с дверями, ванной и унитазом — и тоже получил
 * вопрос «кто вы»; переписал заново короче, и подробности пропали.
 *
 * Цена отменяет заявку только там, где кроме цены ничего нет: «по ценам
 * сами да» — разговор исполнителя, а «нужно вывезти диван, подскажите
 * стоимость» — клиент, и стоимость его волнует ровно как клиента.
 */
/**
 * Расшифровка, в которой слова ходят по кругу. Нормальная речь на
 * двенадцать слов почти вся из разных слов; у сбоя распознавания их
 * пять на двенадцать, и одна и та же связка повторяется.
 */
export function looksLikeGarbledTranscript(text: string): boolean {
  const words = text
    .toLowerCase()
    .replace(/[^a-zа-яёәғқңөұүһі\s]/gi, " ")
    .split(/\s+/)
    .filter((w) => w.length > 1);
...
function looksLikeServiceRequest(text: string): boolean {
  ...
  if (NEED_RE.test(text)) return true;
  if (TASK_RE.test(text) && (PLACE_RE.test(text) || findCitiesInText(text).length > 0)) return true;
  return false;
}
...
/**
 * «Разошлите ещё раз» — просьба клиента возобновить поиск.
 *
 * Заявка №159, +7 700 424 2050. Вечером человек попросил «хватит» — звонков
 * не было, и он решил, что мы зря его беспокоим. Утром написал «Можно
 * сделать рассылку», следом «Не нашел», и оба раза получил «исполнители
 * перезвонят вам сами» — при том что рассылка стояла на паузе и звонить было
 * некому. Возобновить её словами клиент не мог: такая команда была только у
 * админа.
 *
 * Ловим и просьбу («можно рассылку»), и требование («отправьте повторно»), и
 * жалобу с намёком («никто не позвонил, разошлите ещё»).
 */
const RESEND_RE =
  /(разошл|рассыл|отправ|перешл|скинь|разосл)[а-яё]*[\s]*(её|ее|заявку|повторно|ещ[её]|снова|заново|опять)?|(ещ[её]|снова|заново|опять|повторно)[\s]*(раз)?[\s]*(разошл|отправ|рассыл)[а-яё]*|қайта[\s]*(жібер|тарат)[а-яё]*|тағы[\s]*(жібер|тарат)[а-яё]*/i;
...
// Голое «нашла» — тоже исход. Заявка №175, 9 сентября: клиентка написала
// «Отмените», потом «Нашла» — и получила «исполнители перезвонят вам сами».
// Перечень знал «нашёл исполнителя» и «уже нашёл», но не одно слово и не
// женский род.
const FOUND_EXECUTOR_RE =
  /(наш[её]л|нашл[аи]|найден|подобрал|определил)[а-яё]*\s*(уже\s*)?(исполнител|подрядчик|мастера|машину|технику|человека|бригаду)|уже\s*(наш[её]л|нашл[аи]|договорил|заказал|решил)|^\s*(уже\s*)?(наш[её]л|нашл[аи])(\s*сам[аи]?)?[\s.!]*$|договорил[а-яё]*\s*(уже\s*)?(с|уже)?|вопрос\s*реш|всё\s*реш|все\s*реш|^\s*готово[\s.!]*$|таптым|келістім|шештім|^\s*дайын[\s.!]*$/i;
...
/**
 * Вопрос о том, какие заявки будут приходить.
 *
 * Одного упоминания заявки мало — оно есть и в «сколько по этой заявке», —
 * поэтому применяется только вместе с looksLikeQuestion().
 */
...
/**
 * Работа сделана. Берик, 12 сентября, 00:26: «Заказ выполнил» — и в ответ
 * «Записал по заявке №183. Если нужен живой человек». Это не комментарий,
 * это конец заявки: клиента пора спросить, всё ли в порядке, и закрыть.
 */
const DONE_RE =
  /выполнил|выполнен|сделал[аи]?(?![а-яё])|отработал|закончил|завершил|заказ\s*закрыт|закрыл[аи]?\s*заказ|орындалды|орындадым|бітті|аяқталды|жасадым/i;

/**
 * Клиент нашёл другого. +7 705 631 85 46, 13 сентября: «Нашли говорят они
 * сами рахмет» — исполнитель звонил, а заявка уже закрыта с другой
 * стороны. Для нас это тоже исход: заявку закрыть, у клиента уточнить.
 */
const CLIENT_FOUND_ELSEWHERE_RE =
  // Между «нашли» и «сами» бывают слова: «Нашли говорят они сами» — до трёх.
  /(нашл[иа]?|наш[её]л)\s+(\S+\s+){0,3}(сам[иа]?|други[хм]|другого|другую)(?![а-яё])|(сам[иа]?|уже)\s+(\S+\s+){0,2}нашл|(другого|другую|других)\s*(нашл|взял|наня)|клиент\s*(уже\s*)?(нашл|наш[её]л|договорил)|өзі\s*тапты|тауып\s*алды|басқаны\s*тапты/i;
...
export class WhatsAppRouterService {
  ...
  private async handleToken(chatId: string, phone: string, token: string, lang: Language): Promise<void> {
    ...
    // Кнопка из вопроса, который давно закрыт.
    //
    // Кнопки в WhatsApp живут вечно: человек листает переписку вверх и
    // нажимает ту, что была задана вчера. Шынгыс, 1 сентября: через две
    // минуты после конца регистрации нажал «Моей нет в списке» из вопроса
    // про категории — и не получил ничего. Молчание в ответ на нажатие
    // читается как поломка, а не как «эта кнопка уже не нужна».
    //
    // Отвечаем всегда, даже когда сказать нечего: «нажатие не потерялось» —
    // само по себе ответ.
    this.logger.log(`${phone}: нажата кнопка закрытого вопроса (${token})`);
    if (kind === "sup") {
      await this.whatsapp.sendText(
        phone,
        lang === "kk"
          ? "Бұл — жабылған сұрақтың батырмасы, профиліңіз сақталған.\nҚызмет немесе қала қосу үшін «профиль» деп жазыңыз."
          : "Это кнопка из вопроса, который уже закрыт — ваш профиль сохранён.\nЧтобы добавить услугу или город, напишите «профиль».",
      );
      return;
    }
    await this.whatsapp.sendText(
      phone,
      lang === "kk"
        ? "Бұл батырма ескі сұрақтан — қазір жарамсыз. Не керек екенін жазып жіберіңіз."
        : "Эта кнопка из старого сообщения и уже не действует. Напишите словами, что нужно.",
    );
  }
  ...
  private async recordOrderReply(
    ...
    // Порядок — по тому, что требует действия. Недозвон важнее согласия:
    // «взял, но не берут телефон» — это не сделка, а просьба о помощи.
    // Конец заявки («выполнил», «клиент нашёл сам») важнее согласия по той
    // же причине: он меняет статус заявки, а согласие — только запись.
    const done = !unreachable && DONE_RE.test(text);
    const foundElsewhere = !unreachable && !done && CLIENT_FOUND_ELSEWHERE_RE.test(text);
    const agreed = !unreachable && !done && !foundElsewhere && AGREED_RE.test(text);
    const declined = !unreachable && !done && !foundElsewhere && !agreed && DECLINED_RE.test(text);
    const outcome = unreachable
      ? "unreachable"
      : done
        ? "done"
        : foundElsewhere
          ? "client_found_elsewhere"
          : agreed
            ? "agreed"
            : declined
              ? "declined"
    ...
    // Что мы уже отвечали этому человеку по этой заявке. Берик, 12
    // сентября: шесть сообщений подряд — шесть одинаковых «Записал по
    // заявке №183. Если нужен живой человек». Повтор дежурной фразы
    // читается как автоответчик; второй раз подряд отвечаем коротко.
    const previous = await this.prisma.supplierOrderReply.findFirst({
    ...
    if (done || foundElsewhere) {
      // Заявку закрывает клиент, а не исполнитель: слово одной стороны —
      // повод спросить другую, а не менять статус. Спрашиваем тем же
      // сообщением с кнопками, что и плановая проверка, и не чаще раза в
      // окно — иначе двое исполнителей, написавших «выполнил», дали бы
      // клиенту два одинаковых вопроса.
      const askedAt = await this.orders.outcomeAskedAt(order.id);
      const askedRecently = !!askedAt && Date.now() - askedAt.getTime() < OUTCOME_ASK_WINDOW_HOURS * 60 * 60 * 1000;
      if (!askedRecently) {
        try {
          await this.orders.sendCompletionCheckin(order.id);
          await this.orders.markOutcomeAsked(order.id);
        } catch (err) {
          this.logger.warn(`Не удалось спросить клиента об исходе №${n}: ${(err as Error).message}`);
        }
      }
      await this.whatsapp.sendText(
        phone,
          ...
          ? lang === "kk"
            ? `Рахмет! №${n} өтінім орындалды деп белгіледім. Клиенттен бәрі дұрыс па деп сұраймын — сосын жабамын.`
            : `Спасибо! Отметил: заявка №${n} выполнена. Спрошу клиента, всё ли в порядке, и закрою её.`
          : lang === "kk"
            ? `Түсіндім, клиент басқа орындаушы тапты. Айтқаныңызға рахмет — №${n} өтінімді тексеріп, жабамын.`
            : `Понял, клиент нашёл другого исполнителя. Спасибо, что сказали — уточню у него и закрою заявку №${n}.`,
      );
      return true;
    }
    if (unreachable) {
    ...
    const reply = agreed
      ...
      : declined
        ...
        : previous?.outcome === "comment" && Date.now() - previous.createdAt.getTime() < 24 * 60 * 60 * 1000
          ? lang === "kk"
            ? "Қабылдадым."
            : "Принял."
          : lang === "kk"
  ...
  /**
   * Что именно человек будет получать: его категории и его города.
   *
   * Отвечаем перечнем, а не общими словами. Вопрос «кроме мусора ничего не
   * будет?» возникает ровно потому, что человек не видит, на что подписан, —
   * и список закрывает вопрос лучше любого объяснения.
   *
   * Про пустой город молчать нельзя: если заявок в его городе у нас пока не
   * было, он это и так заметит — по тишине, — и уйдёт с мыслью, что сервис
   * не работает.
   */
  private async answerSupplierScope(
    phone: string,
    supplier: {
      id: string;
    ...
    const names = supplier.categories
      .map((c) => (c.category.name as { ru?: string; kk?: string })?.[lang === "kk" ? "kk" : "ru"] ?? "")
      .filter(Boolean);
    const cities = supplier.serviceAreas.map((a) => a.city).filter(Boolean);

    const lines: string[] = [];
    lines.push(
      lang === "kk"
        ? "Сізге тек өз санатыңыз бен қалаңыз бойынша өтінімдер келеді — олар араласпайды."
        : "Вам приходят заявки только по вашим категориям и только по вашим городам — они не смешиваются.",
    );
    if (names.length) {
      lines.push((lang === "kk" ? "Санаттарыңыз: " : "Ваши категории: ") + names.join(", ") + ".");
    }
    if (cities.length) {
      lines.push((lang === "kk" ? "Қалалар: " : "Города: ") + cities.join(", ") + ".");
    }
    lines.push(
      lang === "kk"
        ? "Өзгерту керек болса — «профиль» деп жазыңыз."
        : "Нужно изменить — напишите «профиль».",
    );
    ...
    if (attached) {
      ...
      if (FINISHED_STATUSES.includes(order.status)) {
        ...
        if (/нов(ая|ый)\s*(заявк|заказ)/i.test(text) || /жаңа\s*өтінім/i.test(text)) {
          ...
          // «Разошлите ещё раз» — снимаем паузу и продолжаем поиск.
          //
          // Проверяется ПЕРЕД «хватит»: во фразе «хватит ждать, разошлите
          // ещё» есть оба слова, но просят здесь работы, а не тишины.
          if (dto.status === "PUBLISHED" && RESEND_RE.test(text)) {
            await this.resumeDispatchForClient(phone, attached, dto.number, lang);
            return;
          }
          ...
          if (dto.status === "PUBLISHED") {
            ...
            // Рассылка на паузе — обещать звонки нельзя.
            //
            // Заявка №159: вечером человек написал «хватит», утром — «Не
            // нашел», и получил «исполнители перезвонят вам сами». Звонить
            // было некому: он сам остановил рассылку, а снять паузу словами
            // не мог. Говорим, как есть, и предлагаем продолжить.
            if (await this.orders.isDispatchPaused(attached)) {
              await this.whatsapp.sendText(
                phone,
                lang === "kk"
                  ? `№${dto.number} өтінім бойынша жіберу тоқтатылған — сіз «болды» деп жазғансыз.\n` +
                    "Іздеуді жалғастырайын ба? «Қайта жіберіңіз» деп жазыңыз."
                  : `Рассылка по заявке №${dto.number} остановлена — вы просили больше не звонить.\n` +
                    "Продолжить поиск? Напишите «разошлите ещё раз».",
              );
              return;
            }
    ...
    if (!currentOrderId && !NEW_ORDER_PHRASES.test(text) && !looksLikeServiceRequest(text)) {
      ...
      if (supplier) {
        ...
        if (supplier.confirmedAt) {
          ...
          //
          // Шынгыс, 1 сентября: «На газель другие заказы не дадут кроме
          // мусора что ли». Вопрос ушёл в описание техники, а человек
          // получил «учтём при подборе заявок» — ответ не на его вопрос.
          // Через минуту он отключил рассылку.
          //
          // Проверяется ПОСЛЕ разговора о конкретной заявке: «сколько платят
          // по этой?» — это про заказ, а не про то, как устроен подбор.
          if (looksLikeQuestion(text) && SUPPLIER_SCOPE_RE.test(text)) {
            await this.answerSupplierScope(phone, supplier, lang);
            return;
          }
...
Если срочно, звоните напрямую: ${env.supportPhone}`,
   ...
   */
  /**
   * Возобновить поиск по просьбе клиента.
   *
   * Снимаем паузу и сразу запускаем следующую волну: человек просит не
   * «включить настройку», а чтобы ему позвонили. Ждать очередного тика
   * очереди значит ответить «сделаю» и не сделать.
   *
   * Говорим числом, скольким заявка уже уходила: клиент №159 полночи ждал
   * звонков, которых не могло быть, и повторить эту историю нельзя.
   */
  private async resumeDispatchForClient(
    phone: string,
    orderId: string,
    number: number,
    lang: Language,
  ): Promise<void> {
    const already = await this.orders.countDispatchedSuppliers(orderId).catch(() => 0);
    try {
      await this.orders.resumeDispatch(orderId);
      await this.matching.sendWave(orderId);
    } catch (err) {
      this.logger.error(`Не удалось возобновить рассылку по заявке ${number}: ${(err as Error).message}`);
      await this.whatsapp.sendText(
        phone,
        lang === "kk"
          ? `№${number} өтінім бойынша қайта жіберу сәтсіз аяқталды. Операторға хабарладым.`
          : `Не получилось перезапустить рассылку по заявке №${number}. Передал оператору — он разберётся.`,
      );
      // Оператору — через тот же путь, что и просьба о живом человеке, но
      // без второго сообщения клиенту: он уже прочитал, что произошло.
      await this.newOrderAlert.alertWantsHuman(phone, `не удалось возобновить рассылку по заявке №${number}`);
      return;
    }
    const seen =
      already > 0
        ? lang === "kk"
          ? `Бұған дейін өтінімді ${already} орындаушы алды. `
          : `До этого заявку получили ${already} исполнител${supplierPlural(already)}. `
        : "";
    await this.whatsapp.sendText(
      phone,
      lang === "kk"
        ? `Жарайды, №${number} өтінім бойынша іздеуді жалғастырдым. ${seen}Жаңа орындаушыларға жібердім — қоңырауларды күтіңіз.`
        : `Хорошо, продолжил поиск по заявке №${number}. ${seen}Отправил её новым исполнителям — ждите звонков.`,
    );
  }
...
Понадобится техника — просто напишите сюда.`,
  ...
  private async handleVoice(
    ...
    // Распознаватель речи на казахскую речь иногда отвечает русским
    // набором слов по кругу. +7 705 631 85 46, 9 сентября: «Оба нашли
    // говорят, оба нашли говядину. Оба нашли девушку говорят, нашли
    // говядину» — и это ушло в профиль как описание техники. Такой текст
    // никуда не отправляем и не разбираем: честнее попросить написать.
    if (looksLikeGarbledTranscript(text)) {
      this.logger.warn(`${phone}: расшифровка похожа на бессмыслицу, прошу написать текстом`);
      await this.whatsapp.sendText(
        phone,
        lang === "kk"
          ? "Дауыстық хабарламаны дұрыс түсінбедім. Жазып жіберіңізші."
          : "Голосовое разобрал плохо. Напишите, пожалуйста, текстом.",
      );
      return;
    }
