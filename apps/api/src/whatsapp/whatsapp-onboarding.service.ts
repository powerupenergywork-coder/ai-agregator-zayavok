...
const SERVICE_QUESTION_RE = new RegExp(
  [
    ...
    // «Сколько стоит?» на шаге регистрации — вопрос о нашей цене для
    // исполнителя, а не о цене услуги: у клиента он ловится отдельно, в
    // PRICE_QUESTION_RE, и сюда не попадает.
    "сколько\\s+(это\\s+)?стоит|қанша\\s+тұрады",
  ...
  async handleIncoming(chatId: string, phone: string, msg: IncomingWhatsAppMessage, lang: Language = "ru"): Promise<void> {
    ...
    if (state.step === "company_name") {
      ...
      if (isOnboardingTrigger(name) || looksLikeQuestion(name)) {
        // Пояснение здесь НЕ отправляем.
        //
        // Вопрос о сервисе перехватывается раньше — отдельной проверкой по
        // теме, в начале handleIncoming. Значит сюда доходит вопрос, к сервису
        // не относящийся, и лекция «мы не нанимаем» ему не по адресу.
        //
        // 1 сентября, +7 778 373 0583: человек спросил «Можна по казахский?»
        // и получил объяснение про наём. Так выглядит проверка, которая ловит
        // вопросительный знак вместо смысла.
        await this.whatsapp.sendText(
          phone,
          lang === "kk"
