...
export const env = {
  ...
  // Через сколько часов простоя закрывать незавершённый черновик.
  //
  // Заявка, которую не дозаполнили, не превращается в заказ ни через сутки,
  // ни через месяц — она просто копится в списке. На 13 августа таких было
  // тридцать, самая старая с 19 июля, и каждая требует внимания оператора,
  // который открывает её и обнаруживает пустоту.
  //
  // Ноль отключает автозакрытие.
  orderDraftExpireHours: num("ORDER_DRAFT_EXPIRE_HOURS", 24),
  ...
  // App Secret приложения в Meta (Settings → Basic → App Secret). Им подписан
  // каждый вебхук: без проверки любой, кто знает адрес, может прислать
  // выдуманное входящее сообщение — а бот на него ответит, потратив наши
  // деньги на OpenAI и на отправку.
  //
  // Пусто = проверка выключена. Это сделано чтобы выкатка не оборвала приём
  // сообщений до того, как секрет попадёт в .env на сервере; при пустом
  // значении в лог на старте пишется предупреждение.
  whatsappCloudAppSecret: str("WHATSAPP_CLOUD_APP_SECRET", ""),
  ...
  // Сколько израсходованных заявок у одного исполнителя — повод сказать
  // владельцу, что скоро понадобится принимать деньги.
  //
  // 40 из 50: остаётся десяток заявок, то есть дни или недели — успеть
  // оплатить тариф и включить выставление счетов. Ноль выключает.
  quotaAlertThreshold: num("QUOTA_ALERT_THRESHOLD", 40),
  ...
  // если живой человек упрётся в лимит, его надо поднять сегодня, а не к
  // следующему деплою.
  //
  // Три уровня — по цене запроса для нас:
  //  * общий: почти всё остальное, только чтобы не выкачивали базу подряд;
  //  * дорогой: заявка и переписка с ИИ — каждый вызов платный в OpenAI;
  //  * отправка кода: каждый вызов — это SMS или сообщение WhatsApp, то есть
  //    прямые деньги. У кода уже есть пауза на повтор по одному номеру, но
  //    она не мешает перебирать чужие номера подряд — вот от этого и лимит.
  throttleWindowSeconds: num("THROTTLE_WINDOW_SECONDS", 60),
  throttleLimit: num("THROTTLE_LIMIT", 120),
  throttleLimitExpensive: num("THROTTLE_LIMIT_EXPENSIVE", 15),
  throttleLimitOtp: num("THROTTLE_LIMIT_OTP", 5),
  // То же самое, но для самого бота: сколько сообщений с одного номера за
  // окно мы готовы обработать. Лимиты выше сюда не достают — входящие
  // приходят через вебхук Meta, а не от того, кто их пишет.
  whatsappFloodLimit: num("WHATSAPP_FLOOD_LIMIT", 20),
  whatsappFloodWindowSeconds: num("WHATSAPP_FLOOD_WINDOW_SECONDS", 60),
  // Защита от медленной петли — того, что лимит выше не ловит.
  //
  // 11 августа наш бот и чужой автоответчик переписывались 1 час 43 минуты:
  // тот шлёт «вы не отвечали 25 минут», наш отвечает «не понимаю», тот
  // считает это активностью и шлёт следующее. Темп — раз в 20–30 секунд, до
  // порога флуда далеко, а конца у этого нет: 24-часовое окно WhatsApp не
  // закроется, пока собеседник пишет.
  //
  // Сколько раз подряд отвечаем «не понимаю» на непонятые сообщения.
  whatsappStuckReplyLimit: num("WHATSAPP_STUCK_REPLY_LIMIT", 1),
  // После скольких непонятых подряд перестаём даже разбирать сообщение —
  // чтобы не платить за вызов ИИ в пустоту.
  whatsappStuckSilenceAfter: num("WHATSAPP_STUCK_SILENCE_AFTER", 5),
  // За сколько часов серия непонятых «стареет». Нужно, чтобы человек,
  // у которого разговор не задался вчера, сегодня получил ответ.
  whatsappMissWindowHours: num("WHATSAPP_MISS_WINDOW_HOURS", 6),
  ...
  // Куда слать оповещение о новой заявке — сразу, а не вечерней сводкой.
  // Пусто = на тот же номер, что и сводка; пусто и там = не слать вовсе.
  newOrderAlertPhone: str("NEW_ORDER_ALERT_PHONE", ""),
  // Через сколько минут после рассылки сказать клиенту, сколько исполнителей
  // открыли заявку. Раньше десяти смысла нет — никто не успеет посмотреть;
  // сильно позже человек уже решил, что сервис не работает. 0 отключает.
  dispatchProgressAfterMinutes: num("DISPATCH_PROGRESS_AFTER_MINUTES", 10),
