...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  // «Уборка снега» стоит здесь, а не только у клининга: слово «уборк» есть
  // у обоих, и без этих связок снег с территории уезжал бы к мойщикам окон.
  "front-loader": [
    "фронтальный погрузчик", "погрузчик", "бобкэт", "бобкат", "bobcat",
    "уборка снега", "убрать снег", "почистить снег", "чистка снега", "вывоз снега",
  ],
  disinfection: ["дезинфек", "дезинсек", "дератиз", "таракан", "клоп", "грызун", "потрав", "санобработ"],
  electrician: ["электрик", "розетк", "проводк", "щиток", "выключател", "люстр", "нет света", "замыкани", "электромонтаж"],
  // «Кран» отдельным словом не берём: он занят автокраном и весит там
  // больше. Сантехнический кран узнаётся только в связке — «течёт кран»,
  // «капает кран», — и эти связки лежат в сильных словах ниже.
  plumber: [
    "сантехник", "засор", "унитаз", "смесител", "канализац", "бойлер", "труб", "протеч", "теч",
    "теч[её]т кран", "кран теч[её]т", "капает кран", "полотенцесушител",
  ],
  cleaning: ["клининг", "уборк", "убрать квартир", "помыть окн", "мойка окон", "генеральная уборка", "химчистк"],
  "tow-truck": ["эвакуатор", "эвакуац", "отбуксир", "буксир", "не заводится", "заглохл"],
};
