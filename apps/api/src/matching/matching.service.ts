...
export class MatchingService {
  ...
  async sendWave(orderId: string) {
    ...
    // Клиенту пишем один раз — когда заявка впервые ДОШЛА до кого-то.
    //
    // Заявка №181, 11 сентября, 21:56: за полчаса девять служебных
    // сообщений — «разослали ещё 5», «прочитали 3 из 10», «разослали ещё
    // 4»… А №182 в тот же вечер: сначала «получат утром, первые звонки
    // после восьми», через семь минут «разослали ещё 5, ждите звонков».
    // Клиент отменил заявку через час. Каждая волна — событие для нас, не
    // для него: ему важно, что звонки будут и когда.
    //
    // «Впервые дошла» — в журнале нет отправок, кроме этой волны. Если
    // первая волна ушла в отложенные, а вторая нашла круглосуточных, то это
    // и есть первая настоящая — и сказать надо, что звонки будут сегодня.
    const earlierReached = await this.prisma.notificationLog.count({
      where: { orderId, templateKey: "order_broadcast_full", supplierId: { notIn: sentNow } },
    });
    if (waveNumber > 1 && earlierReached > 0) {
      this.logger.log(`Заявка №${order.number}: волна ${waveNumber} ушла ${sentNow.length}, клиенту не пишем`);
      this.realtime.emitOrderUpdated(orderId, await this.orders.toDto(orderId));
      return;
    }
    ...
    await this.tellClient(
      ...
      waveNumber === 1
        ...
        : `Нашлись исполнители, которые работают и сейчас: отправил заявку №${order.number} ${sentNow.length} из них${city}.\n\n` +
            "Позвонят в ближайшие 15–30 минут. Договорились — напишите «готово», и я закрою заявку." + later,
      waveNumber === 1
        ...
        : `Қазір жұмыс істейтін орындаушылар табылды: №${order.number} өтінімін ${sentNow.length} орындаушыға жібердім.\n\n` +
            "Олар 15–30 минут ішінде қоңырау шалады. Келісіп алсаңыз — «дайын» деп жазыңыз, өтінімді жабамын." + laterKk,
    );
