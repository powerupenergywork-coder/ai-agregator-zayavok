...
export class OrdersService {
  ...
  async notifyDispatchedSuppliers(
    ...
    // Об отмене исполнителям больше не пишем.
    //
    // Текст был «Заявка №137 отменена клиентом.» — ни категории, ни города.
    // Исполнитель, получивший за неделю пять заявок, по одному номеру не
    // понимает, о какой речь, и читает это как «клиент передумал, а ты зря
    // звонил». Тот, кто заявку даже не открыл, получает укор ни за что.
    // Живой отклик по заявке №120: «Не договорился они сами брат🖐️».
    //
    // Полезной эта рассылка была бы ровно одному человеку — тому, кто уже
    // едет. Кто именно едет, мы не знаем: звонки идут мимо нас, у нас есть
    // только отметка о прочтении. Тринадцать сообщений ради одного
    // предположительного адресата — плохой размен, тем более что каждое
    // тратит рейтинг номера, а пожаловаться на спам проще, чем отписаться.
    //
    // Заслон стоит здесь, а не в четырёх местах вызова: так его не обойдёт и
    // будущий код. Шаблон и вся обвязка на месте — включается обратно
    // удалением этих трёх строк.
    if (event === "order_cancelled") {
      this.logger.log(`Заявка ${orderNumber}: отмену исполнителям не отправляем`);
      return;
    }
