    ...
    <main className="mx-auto min-h-screen max-w-2xl px-4 py-8">
      ...
      <Card className="p-4">
        ...
        <div className="mt-2 flex flex-col gap-1 text-sm text-slate-600">
          {order.city && <p>Город/район: {order.city}</p>}
          {order.dateNeeded && <p>Дата: {new Date(order.dateNeeded).toLocaleDateString("ru-RU")} {order.timeWindow ?? ""}</p>}
      ...
      {order.clientPhone ? (
        <Card className="mt-4 p-4">
          <h2 className="mb-2 text-base font-medium">Контакты клиента</h2>
          <p className="text-sm">
            Позвоните и договоритесь напрямую:{" "}
        ...
        <Card className="mt-4 p-4 text-sm text-slate-500">Заявка ещё не опубликована.</Card>
      )}
    </main>
  );
}
