...
/** Служебные слова вокруг названия города — сами по себе ничего не значат. */
const CITY_STOP_WORDS = new Set([
  "г", "гор", "город", "қала", "обл", "область", "облысы", "район", "аудан",
  "и", "или", "да", "по", "в", "во", "на", "до", "от", "его", "него", "тоже",
  "также", "пригород", "окрестности", "около", "рядом", "плюс", "және",
  "у", "меня", "мы", "я", "нас", "это", "есть",
]);

/**
 * Разметка вокруг названия города — только она вырезается из остатка.
 *
 * Список выше шире: он нужен, чтобы понять, называл ли человек город вообще.
 * Для остатка такой список не годится — «Семей и пригород до 150км от него»
 * потерял бы весь смысл и остался пустым, тогда как радиус выезда мы как раз
 * хотим сохранить в заметке профиля.
 */
const CITY_MARKERS = new Set([
  "г", "гор", "город", "қала", "обл", "область", "облысы", "и", "және", "плюс",
]);
...
export function cityLeftover(chunk: string, found: CityEntry[]): string {
  ...
  const rest = chunk
    ...
    .filter(Boolean)
    .filter((w) => {
      const n = normalizeCityInput(w);
      if (!n) return false;
      if (CITY_MARKERS.has(n)) return false;
      // Само название города и его словоформы — уже учтены.
      return !heads.some((h) => h.length >= 4 && n.slice(0, 4) === h);
    });
  const text = rest.join(" ").trim();
  // Одна буква остатком — хвост разбора, а не смысл. Цифры считаем: «до
  // 150км» без них превращается в бессмысленное «до км».
  return text.replace(/[^0-9a-zA-Zа-яёА-ЯЁәғқңөұүһіӘҒҚҢӨҰҮҺІ]/g, "").length >= 3 ? text : "";
}

/**
 * Похоже ли на попытку назвать город.
 *
 * Нужно, чтобы решить, показывать ли человеку «не узнал город» со списком.
 * На «Кокшетау» показать надо: он назвал город, которого у нас нет. На «на
 * фото кроме шкафов все разобрано» — нет: это описание задачи, попавшее в
 * поле города, и список городов в ответ выглядит так, будто нас не слушают.
 *
 * Всего шесть таких ответов на проде у пяти человек, и ни одного полезного.
 */
export function looksLikeCityAnswer(raw: string): boolean {
  // «Нужен автокран» — два слова, но это заказ, а не город. Слово просьбы
  // выдаёт описание задачи вернее любой длины: человек, отвечающий на вопрос
  // «в каком городе?», пишет название, а не «нужен».
  if (/(^|\s)(нужен|нужна|нужно|нужны|требуется|надо|хочу|ищу|закажу|керек|қажет|болады)(\s|$)/i.test(raw)) {
    return false;
  }
  const words = raw
    .split(/[\s,]+/)
    .map((w) => normalizeCityInput(w))
    .filter((w) => w && !CITY_STOP_WORDS.has(w) && w.length >= 3);
...
export function resolveCityList(raw: string): CityListResolution {
  ...
  for (const chunk of raw.split(/[,;/\n]+/).map((s) => s.trim()).filter(Boolean)) {
    ...
    // Город назван внутри фразы, а не отдельным словом.
    //
    // 31 августа человек написал «г Семей область Абай», потом «Семей и
    // пригород до 150км от него» — и оба раза услышал «Не узнал город».
    // Семей есть в справочнике, там двое наших исполнителей. Разбор выше
    // требует, чтобы КАЖДОЕ слово оказалось городом, а живые люди пишут с
    // областью, с радиусом выезда и с «г» впереди.
    //
    // Ищем строгим сканированием: оно не допускает опечаточных совпадений и
    // на «на фото кроме шкафов все разобрано» не находит ничего. Остаток
    // всё равно кладём в unresolved — «пригород до 150 км» стоит сохранить,
    // это зона выезда, а не мусор.
    const inside = findCitiesInText(chunk);
    if (inside.length > 0) {
      inside.forEach(push);
      const leftover = cityLeftover(chunk, inside);
      if (leftover) unresolved.push(leftover);
      continue;
    }
